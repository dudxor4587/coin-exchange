...
## 인메모리였다면 얼마나 잃는가
"durable 하니 안 잃는다"만 보이면 한쪽만 측정한 것이다. 7단계 구조(인메모리 @Async)로 되돌려 같은 조건으로 유실을 재현했다. <br>
projection이 밀린 상태에서 크래시하도록, trading-db를 `docker pause`로 멈춰 큐를 쌓은 뒤 800건을 접수시키고 trading을 하드킬했다. <br>
...
7단계에서는 접수된 800건이 인메모리 큐와 함께 통째로 사라졌고, 8단계에서는 로그에 남아 재시작 후 전부 복구됐다. <br>
