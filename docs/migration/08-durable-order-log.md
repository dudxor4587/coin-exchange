...
"유실될 수 있다"는 논리만으로 넘어가지 않고, 실제로 얼마나 잃는지부터 쟀다. <br>
projection이 밀린 상태에서 크래시하도록 trading-db를 `docker pause`로 멈춰 큐를 쌓았다. <br>
그 상태로 800건을 접수시키고 trading을 하드킬(SIGKILL)한 뒤 재시작했다. <br>

접수 800건, DB 반영 0건. **접수된 800건이 인메모리 큐와 함께 통째로 사라졌다.** <br>
7단계 구조에는 이를 복구할 수단이 없다. <br>
이 800건이 다음 개선의 근거다. <br>
...
# 유실 검증 — 같은 크래시, 0건
구멍을 확인했던 조건 그대로(trading-db `docker pause` → 800건 접수 → 하드킬 → 재시작) durable 구조를 측정했다. <br>
...
같은 크래시에 800 대 0이다. <br>
8단계는 접수 시점에 Kafka에 durable append됐으니, 재시작한 컨슈머가 로그의 offset부터 재개해 800건을 전부 복구했다. <br>

부하 중 하드킬로도 확인했다. 6655건이 로그에 쌓인 상태에서 죽였는데, 재시작 후 컨슈머 lag이 0으로 떨어지며 전부 반영됐고 중복 order도 0이었다(dedup). <br>
