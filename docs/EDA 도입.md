...
하지만 이 구조에도 큰 문제가 존재한다.
데이터 정합성의 측면에서 보았을 때, 물론 현재 코드로는 이벤트 발행이 실패한다면 롤백이 되니 문제가 없다. <br>
하지만 이벤트 발행은 성공했는데, 사용자의 잔고 업데이트나 알림 전송이 실패한다면? <br>
이 경우에는 입금 요청은 승인되었지만, 사용자의 잔고는 업데이트 되지 않고 알림 전송은 실패하게 된다. <br>
우려했던 상황이 발생하게 된다. <br>
...
---
우선 단순히 생각했을 때, 이벤트 발행 자체를 afterCommit으로 처리하면 되지 않을까? <br>
이벤트 발행을 afterCommit으로 처리하면, 트랜잭션이 커밋된 후에 이벤트가 발행되기 때문에, 문제가 생기지 않는다. <br>
하지만, 이벤트 수신자의 입장에서 보면, 이벤트를 수신했더라도 로직을 처리하다 문제가 발생하면, 입금 요청은 승인 상태로 남아있지만, 사용자의 잔고는 업데이트 되지 않는다. <br>
이벤트 기반으로 구현했기 때문에 발생하는 일이며, 오류가 발생했을 때 어떻게 처리할 것인지에 대한 고민이 필요하다. <br>
<br>
두 가지 정도 생각해 보았다.
1. 이벤트를 수신한 후, 사용자의 잔고를 업데이트하고 알림을 전송하는 과정에서 문제가 발생하면, 해당 이벤트를 다시 발행한다.
2. 오류로 실패 시에 이벤트를 다시 발행하는 것이 아니라, 실패한 이벤트를 DB에 저장하고, 이후에 다시 처리하는 방법이다.

어떤 방법이 더 좋을까? <br>
사실 두 가지 방법 모두 장단점이 있다. <br>
첫 번째 방법은 이벤트를 다시 발행하는 것이기 때문에, 이벤트를 수신한 서비스가 중복 처리를 해야 한다. **멱등성**이 너무나도 중요해진다. <br>
두 번째 방법은 이벤트를 다시 발행하지 않기 때문에, 중복 처리를 하지 않아도 된다. <br>
하지만, 이벤트를 DB에 저장해야 하기 때문에, DB의 부하가 증가할 수 있다. <br>

<br>
그래도 "입금"이라는 비지니스 로직은 정합성이 매우 중요하기 때문에, 두 번째 방법이 더 적합하다고 생각했다. <br>

> EDA는 분명 큰 단점이 존재하지만, 각 서비스들이 독립적으로 동작할 수 있다는 장점이 있다. <br>
