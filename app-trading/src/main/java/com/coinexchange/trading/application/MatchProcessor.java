 ...
 * Trade INSERT / Order fill / 알림은 TradeExecutedEvent를 durable 로그(Kafka)에
 * 동기 append한 뒤 컨슈머가 DB에 반영한다. 체결의 진실은 이미 매칭 Lua가
 * Redis OrderBook에 반영했으므로(remainingAmount 차감), DB 기록을 hot path에서
 * 동기로 기다릴 이유가 없다.
 */
@Component
...
public class MatchProcessor {
    ...
    public void processMatch(Map<String, Object> match) {
        ...
        orderLogPublisher.appendTradeExecuted(new TradeExecutedEvent(
